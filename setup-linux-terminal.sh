#!/usr/bin/env bash
# Setup para la app "Terminal" nativa de Android (Linux Terminal / AVF).
# Distinta de Termux + proot-distro (ver setup-debian.sh): esta es una VM Debian
# real de Google, sin relación con Termux, activada en:
#   Ajustes > Sistema > Opciones de desarrollador > "Linux development environment"
#
# Se ejecuta DENTRO de la app Terminal (usuario "droid", con sudo).
...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════╗"
echo "║  Linux Terminal (AVF) Dev Setup      ║"
echo "╚══════════════════════════════════════╝"
echo -e "${NC}"

[ -f /etc/debian_version ] || err "Esto no parece Debian. Ejecuta esto dentro de la app Terminal de Android."
[ -d /mnt/shared ] || warn "No se encontró /mnt/shared — ¿es esta realmente la app Terminal de Android?"
...
install_editors() {
    ...
    sudo apt install -y neovim vim nano
}

# ── Lenguajes de programación ────────────────────────────────────────────────────
install_languages() {
    log "Instalando lenguajes de programación..."
    sudo apt install -y python3 python3-pip python3-venv golang rustc cargo
...
install_shell() {
    ...
    cp -f "$(dirname "$0")/dotfiles/.zshrc-linux-terminal" "$HOME/.zshrc" 2>/dev/null || true

    sudo chsh -s "$(command -v zsh)" "$USER" 2>/dev/null || warn "Cambia el shell manualmente con: chsh -s zsh"
}
...
setup_ssh() {
    ...
    if [ -f "$key" ]; then
        ...
        mkdir -p "$HOME/.ssh"
        chmod 700 "$HOME/.ssh"
        ssh-keygen -t ed25519 -C "linux-terminal-android" -f "$key" -N ""
    fi
...
summary() {
    ...
    echo ""
    info "Para pasar archivos entre Android y esta VM: usa la carpeta Descargas ↔ /mnt/shared."
    warn "Cierra y vuelve a abrir la app Terminal para aplicar el shell."
}
